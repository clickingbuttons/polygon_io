...
impl Client {
  ...
  pub fn get_us_tickers(&mut self, page: usize) -> std::io::Result<TickersResponse> {
  ...
  pub fn get_all_tickers_vx(&mut self, date: &NaiveDate) -> std::io::Result<Vec<TickerVx>> {
    let limit: usize = 500;
...
mod tickers {
  ...
  fn works_vx_day() {
    let client = Client::new();
    let results = client
      .get_all_tickers_vx(&NaiveDate::from_ymd(2004, 01, 02))
      .unwrap();
